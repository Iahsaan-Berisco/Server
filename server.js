...
app.post('/api/pcs/:pcId/session/start', authMiddleware, async (req, res) => {
  try {
    ...
    // FIX: Clear stopwatch_start when starting timed session
    ...
    // FIX: Emit session update with BOTH fields for cross-device sync
...
app.post('/api/pcs/:pcId/session/add-time', authMiddleware, async (req, res) => {
  try {
    ...
    // FIX: Emit session update with BOTH fields
    io.to(`group:${group_id}`).emit('group:'+group_id+':pc-session', { 
      pc_id: pcId, 
      session_end: new_end, 
...
app.post('/api/pcs/:pcId/session/end', authMiddleware, async (req, res) => {
  try {
    ...
    if (!await canManageGroup(req.user.id, group_id)) return res.status(403).json({ error: 'Forbidden' });
    // FIX: Clear BOTH session_end AND stopwatch_start when ending session
    ...
    // FIX: Emit session update with BOTH fields cleared for cross-device sync
    io.to(`group:${group_id}`).emit('group:'+group_id+':pc-session', { 
      pc_id: pcId, 
      session_end: 0, 
      stopwatch_start: 0 
...
app.post('/api/pcs/:pcId/session/stopwatch', authMiddleware, async (req, res) => {
  try {
    ...
    // FIX: Clear session_end when starting stopwatch
...
app.post('/api/pcs/:pcId/session/stopwatch-end', authMiddleware, async (req, res) => {
  try {
    ...
    const { group_id } = req.body;
    if (!await canManageGroup(req.user.id, group_id)) return res.status(403).json({ error: 'Forbidden' });
    // FIX: Clear BOTH fields when ending stopwatch
    await db.update('pcs', p => p.id === pcId, { session_end: 0, stopwatch_start: 0 });
    // FIX: Emit session update with BOTH fields cleared
    io.to(`group:${group_id}`).emit('group:'+group_id+':pc-session', { 
      pc_id: pcId, 
      session_end: 0, 
      stopwatch_start: 0 
...
io.on('connection', (socket) => {
  socket.on('pc:auth', async ({ pc_name, group_id, password }, callback) => {
    ...
    const now = Math.floor(Date.now()/1000);
    // FIX: Only send valid stopwatch_start (in the past)
